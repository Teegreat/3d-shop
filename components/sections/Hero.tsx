...
const Hero = () => {
  return (
    <div
      id="hero"
      className="relative h-screen max-w-5xl mx-auto flex flex-col p-4 pt-8 lg:pt-32 md:16"
    >
      <div className="flex flex-col md:flex-row items-center relative">
        <div className="w-full h-full md:w-1/2 flex flex-col gap-8 z-10 bg-gradient-to-r from-stone-950 via-stone-950 to-transparent">
          ...
          <div className="flex flex-row gap-6">
            ...
            <Link
              href="#"
              className="w-36 flex justify-center py-3 rounded-xl text-xs border border-white"
            >
              See more
            </Link>
          </div>
        ...
        <Image
          ...
          height={400}
          unoptimized={true}
          className="left-1/2 top-4 mt:16 md:mt-0 md:absolute"
          priority
        />
      </div>
      <Link
        href="#catalog"
        className="absolute bottom-24 left-1/2 transform -translate-x-1/2 flex items-center px-6 py-2 mx-auto mb-16 mt-auto border border-slate-400 rounded-xl text-sm text-slate-400"
      >
        <MdKeyboardDoubleArrowDown  className="w-4 h-4 mr-2 animate-ping" />
        <span className="text-sm font-semibold">Click to scroll</span>
